    ...
    >
      ...
      <section style={{ marginBottom: 16 }}>
        ...
        >
          <input
            value={tcpMsg}
            onChange={(e) => setTcpMsg(e.target.value)}
            style={{ width: 360 }}
          />
          <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <input
              ...
              onChange={(e) => setExpectResponse(e.target.checked)}
            />
            Expect Response
          </label>
          <button
            onClick={() =>
              tcpSend(tcpMsg, /*appendNewline:*/ true, {
                expectResponse,
                perCmdTimeoutMs,
              })
            }
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <button onClick={() => measureCurr()} style={{ marginLeft: 8 }}>
            Measure Current
          </button>
        </div>
      </section>
      ...
      <section style={{ marginBottom: 16 }}>
        ...
        >
          ...
          {speedMode === 'rtt' ? (
            <>
              ...
              <input
                type="number"
                value={speedCount}
                onChange={(e) => setSpeedCount(Number(e.target.value))}
                style={{ width: 80 }}
              />
            </>
          ) : (
          ...
          <input
            type="number"
            value={perCmdTimeoutMs}
            onChange={(e) => setPerCmdTimeoutMs(Number(e.target.value))}
            style={{ width: 100 }}
          />
          ...
          <input
            ...
            onChange={(e) => setMaxConsecutiveTimeouts(Number(e.target.value))}
            style={{ width: 80 }}
          />
        ...
        <p style={{ fontSize: 12, color: '#555', marginTop: 8 }}>
          Speed test uses safe send→await-reply loops (suitable for SCPI
          instruments like BK9801).
        </p>
      </section>
