...
export function useSerial(socketUrl = API_BASE_URL) {
  ...
  useEffect(() => {
    socket.on('serial_rx', (line) =>
      setLines((prev) => [...prev, `RX: ${line}`])
    );
    socket.on('serial_rx_raw', (arr) =>
      setLines((prev) => [...prev, `RX(raw): [${arr.join(', ')}]`])
    );
    socket.on('serial_tx', (line) =>
      setLines((prev) => [...prev, `TX: ${line}`])
    );
    socket.on('serial_err', (msg) =>
      setLines((prev) => [...prev, `ERR: ${msg}`])
    );
    socket.on('serial_open', (info) =>
      setLines((prev) => [...prev, `OPEN: ${JSON.stringify(info)}`])
    );
  ...
  // REST helpers
  const listPorts = () => safeFetch('/serial/ports');
  ...
  const close = () => safeFetch('/serial/close', { method: 'POST' });
